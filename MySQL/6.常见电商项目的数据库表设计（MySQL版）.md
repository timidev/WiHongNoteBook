...
仓配：库存、管理
### 2.用户表涉及的实体
![](https://ws2.sinaimg.cn/large/006tKfTcgy1g0t2xya4grj30rs0aftal.jpg)
#### 2.1改进1：第三范式：将依赖传递的列分离出来
比如：登录名<-用户级别<-级别积分上限，级别积分下限
![](https://ws2.sinaimg.cn/large/006tKfTcgy1g0t2ykto38j30rs0cu411.jpg)
改进2：尽量做到冷热数据的分离，减小表的宽度
![](https://ws3.sinaimg.cn/large/006tKfTcgy1g0t2yvtbwzj30rs0edq5l.jpg)
### 3.表设计
#### 3.1 用户登录表(customer_login)
...
```
#### 3.5 用户积分日志表(customer_point_log)
...
```
#### 3.6 用户余额变动表(customer_balance_log)
```
...
```
#### 3.7 用户登陆日志表(customer_login_log)
```
...
```
转载自[linxinzhe](https://www.jianshu.com/p/b89127a415df)
